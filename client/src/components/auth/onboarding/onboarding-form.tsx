...
export const OnboardingForm = () => {
  ...
  const { data: schools, isPending: fetchingSchools } = useQuery({
    ...
    queryFn: async () => {
      if (!collegeId) return [];
      const res = await axiosInstance.get("/accounts/schools/", {
  ...
  const { data: departments, isPending: fetchingDepartments } = useQuery({
    ...
    queryFn: async () => {
      if (!schoolId) return [];
      const res = await axiosInstance.get("/accounts/departments/", {
  ...
  const { data: courses, isPending: fetchingCourses } = useQuery({
    ...
    queryFn: async () => {
      if (!schoolId) return [];
      const res = await axiosInstance.get("/accounts/courses/", {
        params: { school_id: schoolId },
      });
      return res.data as { id: number; name: string }[];
    },
  ...
  const { mutate: onSubmit, isPending: updatingProfile } = useMutation({
    mutationFn: async (values: OnboardingFormValues) => {
      ...
      checkAuthStatus().then(() => navigate(DEFAULT_LOGIN_REDIRECT));
      return res.data;
    },
    onError: (error) => {
              ...
              <FormControl>
                ...
                >
                  <SelectTrigger className="w-full">
                    <SelectValue
                      placeholder={
                        fetchingColleges ? "Loading..." : "Select College"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <FormItem>
                ...
                <FormControl>
                  <Select
                    value={field.value}
                    onValueChange={field.onChange}
                    disabled={fetchingSchools || !collegeId}
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue
                        placeholder={
                          collegeId && fetchingSchools
                            ? "Loading..."
                            : "Select School"
                        }
                      />
              ...
              <FormItem>
                ...
                <FormControl>
                  <Select
                    value={field.value}
                    onValueChange={field.onChange}
                    disabled={fetchingDepartments || !schoolId}
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue
                        placeholder={
                          schoolId && fetchingDepartments
                            ? "Loading..."
                            : "Select Department"
                        }
                      />
              ...
              <FormItem>
                ...
                <FormControl>
                  <Select
                    value={field.value}
                    onValueChange={field.onChange}
                    disabled={fetchingCourses || !schoolId}
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue
                        placeholder={
                          schoolId && fetchingCourses
                            ? "Loading..."
                            : "Select Course"
                        }
                      />
