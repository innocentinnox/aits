...
const formSchema = z.object({
  ...
  first_name: z.string().min(2, "First name must be at least 2 characters").max(30),
  last_name: z.string().min(2, "Last name must be at least 2 characters").max(30),
  username: z.string()
...
export const SignupForm = ({ className }:{ className?: string }) => {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit((values) => onSubmit(values))} className={cn("space-y-2", className)}>
        <FormField
        ...
        />
        <Button type="submit" className="w-full !mt-4" disabled={loading}>Signup</Button>
      </form>
    </Form>
  )
}
