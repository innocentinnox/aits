...
const fileWithPreviewSchema = z.custom<FileWithPreview>(
  (val) => {
...
export const CreateIssueForm = ({
  ...
  const { data: course_units, isPending: fetchingcourse_units } = useQuery({
    ...
    queryFn: async () => {
      if (!yearTaken) return [];
      const res = await axiosInstance.get("/accounts/course-units/", {
        params: { course_id: user?.course?.id, year_taken: yearTaken },
      });
            ...
            render={({ field }) => (
              <FormItem className="flex-[0.8]">
                ...
                <Select
                  ...
                  value={field.value}
                  disabled={fetchingcourse_units || !yearTaken}
                >
                  <SelectTrigger className="w-full">
                    <SelectValue
                      placeholder={
                        yearTaken && fetchingcourse_units
                          ? "Loading..."
